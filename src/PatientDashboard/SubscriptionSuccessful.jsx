...
import axios from "axios";
import { getToken } from "../utils";
import { baseUrl } from "../env"; // make sure this points to your backend
...
export default function SubscriptionSuccessful() {
  ...
  const verifyPayment = async (ref) => {
    try {
      ...
      // Flexible check depending on backend response
      if (
        res.data?.status === "success" ||
        res.data?.data?.status === "success" ||
        res.data?.success === true
        ...
        setVerified(false);
        setError(
          `Unexpected response: ${JSON.stringify(res.data, null, 2)}`
        );
      }
    } catch (err) {
      ...
      <div className="bg-white rounded-2xl shadow-lg p-8 max-w-md w-full text-center">
        ...
        {!loading && verified && (
          <p className="mt-3 text-gray-600 text-sm md:text-base">
            ...
            enjoy all the premium features available to you.
          </p>
        )}
