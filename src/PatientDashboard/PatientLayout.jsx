...
export default function PatientLayout() {
  ...
  function makePaymentToast(message) {
    toast.success(message, {
      ...
      closeOnClick: false,
      rtl: false,
      progressStyle: { background: "#000080" },
      pauseOnHover: false,
      draggable: false,
      pauseOnFocusLoss: false,
    });
  ...
  const handleSubmit = async (e, subtitle) => {
    e.preventDefault();
    const userData = JSON.parse(localStorage.getItem("userData"));
    if (!userData?.emailAddress) return;

    const validAmounts = [1500, 5000, 4500];
    const amountInKobo = parseFloat(subtitle);
    if (!validAmounts.includes(amountInKobo)) return;

    const data = { email: userData.emailAddress, amount: amountInKobo };
    try {
      const response = await axios.post(
