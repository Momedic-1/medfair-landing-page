...
const Subscription = () => {
  ...
  // Enhanced payment verification function with better logging
  const verifyPayment = async (reference) => {
    ...
    try {
      ...
      const verificationUrl = `${baseUrl}/api/payment/verify?reference=${encodeURIComponent(reference)}`;
      console.log("📡 Making verification API call to:", verificationUrl);
  ...
  React.useEffect(() => {
    console.log("🚀 Component mounted, checking for pending payments...");
    ...
    const checkPaymentReturn = async () => {
      ...
      if (reference && amount) {
        console.log("💳 Found pending payment - reference:", reference, "amount:", amount);
        toast.info("Verifying your payment...");
        await verifyPayment(reference);
      } else {
        console.log("ℹ️ No pending payment found in sessionStorage");
      }
    };
  ...
  const handleSubscription = async (e, amount) => {
    ...
    console.log("💰 Initiating subscription payment:", dataToSend);
    ...
    try {
      ...
      if (result) {
        ...
        if (result.reference || result.access_code) {
          const reference = result.reference || result.access_code;
          console.log("💾 Storing payment reference:", reference);
          sessionStorage.setItem("pendingPaymentReference", reference);
          sessionStorage.setItem("pendingSubscriptionAmount", amount.toString());
          
          // Log what was actually stored
          console.log("✅ Stored in sessionStorage:", {
            reference: sessionStorage.getItem("pendingPaymentReference"),
            amount: sessionStorage.getItem("pendingSubscriptionAmount")
          });
        }
    ...
    <div className="w-full px-4 h-screen overflow-auto">
      ...
      {baseUrl === 'development' && (
        <div className="bg-yellow-100 border border-yellow-400 p-4 mb-4 rounded">
          ...
          <p>Pending Reference: {sessionStorage.getItem("pendingPaymentReference") || "None"}</p>
          <p>Pending Amount: {sessionStorage.getItem("pendingSubscriptionAmount") || "None"}</p>
          <button 
